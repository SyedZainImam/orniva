  ...
  return (
    ...
    >
      <div className="relative overflow-hidden" style={{ aspectRatio: "1/1", background: "#1a1a1a" }}>
        ...
        {badge && (
          <div className={`absolute top-2 left-2 px-2 py-[3px] text-[8px] md:text-[9px] font-bold tracking-wider uppercase ${badge.bg}`}>
            {badge.label}
          </div>
        )}

        {discount > 0 && (
          <div className="absolute top-2 right-2 bg-red-sale text-white px-1.5 py-[3px] text-[8px] md:text-[9px] font-bold">
            -{discount}%
          </div>
        )}
